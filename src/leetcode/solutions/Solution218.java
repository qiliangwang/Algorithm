...
public class Solution218 {
    ...
    public List<List<Integer>> getSkyline(int[][] buildings) {
        ...
        List<int[]> buildingInfo = new ArrayList<>();
        for (int[]b :buildings) {
            //left pos with neg
            buildingInfo.add(new int[]{b[0], -b[2]});
            buildingInfo.add(new int[]{b[1], b[2]});
        }
        Collections.sort(buildingInfo, (a, b) -> (a[0] != b[0]) ? a[0] - b[0] : a[1] - b[1]);

        PriorityQueue<Integer> maxHeap = new PriorityQueue<>(Comparator.reverseOrder());
        //default height
        maxHeap.offer(0);
        int preHeight = 0;
        for (int[]b :buildingInfo) {
            if (b[1] < 0) {
                maxHeap.offer(-b[1]);
            } else {
                maxHeap.remove(b[1]);
            ...
            if (preHeight != curHeight) {
                res.add(Arrays.asList(b[0], curHeight));
                preHeight = curHeight;
            }
        }
        return res;
    ...
    public static void main(String[] args) {
        int[][] buildings = new int[][] {
                ...
                {15, 20, 10},
                {19, 24, 8}};
        List<List<Integer>> skyline = new Solution218().getSkyline(buildings);
